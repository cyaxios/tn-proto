...
SDK, backed by one shared core where the runtime allows it and proven
against each other on the wire.

* **TypeScript `tn.seal` / `tn.unseal`.** Pure-TS implementation,
  async-first (JWE sealing and opening are async in TS). Malformed input
  raises `SealedObjectError`; verification failures raise the existing
  `VerifyError` with `failed_checks`. `tn.unseal` works without an active
  ceremony (verify + key-bag walk over whatever keys are on disk).
* **Rust SDK `Tn::seal` / `Tn::unseal`** over the new tn-core
  `Runtime::seal`/`unseal`, with `Error::Verify { failed_checks, sequence,
  event_type }` as a first-class variant.
* **C# `Tn.SealAsync` / `Tn.UnsealAsync`** via new `tn_runtime_seal` /
  `tn_runtime_unseal` C-ABI exports: `SealedObject.RawJson` is the
  transport-verbatim wire string; typed `TnVerifyException` /
  `TnUnsealException`; `ISealedGroupCipher` lets a managed cipher open
  group blocks the native core cannot (the JWE seam).
* **Cross-implementation proof.** Committed golden vectors (generated by
  Python, verified and opened by Rust and TypeScript) plus live
  round-trips in both directions for Python-TypeScript and Python-C#,
  covering aad binding, container and unicode public values, foreign
  objects, and as-recipient kits.
* **Row-hash wire note.** The Rust core now renders list/dict PUBLIC
  field values in the row-hash preimage the way Python always has
  (Python repr, e.g. `[1, 2, 3]` and `{'a': 1}`), instead of compact
  JSON. Rust-written envelopes with container public values previously
  disagreed with Python; Python's rendering is normative. Encrypted
  group fields are unaffected.
* **Key walk completeness.** Pure-Python and TypeScript btn readers now
  open pre-rotation content by walking retired (`.retired.<epoch>`) and
  legacy (`.revoked.<ts>`) kit archives, newest first, matching the
  native runtime. HIBE keys join the Python key-bag, and the bag holds
  per-group candidate lists across ciphers.
* **Governance in C#.** `EmitAsync` takes authenticated `aad` markers
  (echoed as `tn_aad` on read-back) via `tn_runtime_emit_with_aad`, and
  `tn.Agents.PublishAsync` / `Current` ride the core's existing policy
  splice and `tn.agents.policy_published` lifecycle. The Rust SDK gains
  the same surfaces.
* **Sealing hardening (0.6.9 follow-ups).** `tn.seal` rejects public
  field values that a foreign JSON runtime would silently reformat
  (floats, integers beyond 2^53); a JSON round-trip of a sealed object
  in Python and transport of the canonical string are certified safe for
  every value type.
* **CI.** The C# test suite and the dashboard JS suites now gate every
  push alongside the Rust, Python, and TypeScript suites.

## [0.6.2] - 2026-06-15 -- Install everywhere: full wheel matrix + leaner deps

Packaging-only release. No wire-format or public-API changes. The goal is
"`pip install tn-proto` just works on every supported Python and platform."

* **Full platform wheel matrix.** Release now builds eight abi3 wheels per
  version: Linux glibc (`manylinux2014`/glibc 2.17) **and** musl
  (`musllinux_1_2`, Alpine) for both `x86_64` and `aarch64`; macOS `arm64`
  and `x86_64` (Intel); Windows `x86_64` and `aarch64`. Previously only
  `manylinux_2_34_x86_64`, macOS `arm64`, and Windows `x86_64` shipped, so
  Alpine images, ARM Linux, Intel Macs, and any glibc < 2.34 distro
  (Ubuntu 20.04, Debian 11, RHEL/Rocky 8, Amazon Linux 2 / AWS Lambda) got
  "no matching distribution found". Each wheel is abi3-py39, so one file per
  platform covers CPython 3.10–3.14. (32-bit x86 is not built: the
  `cryptography` dependency ships no 32-bit wheels, so it cannot work there
  regardless — use 64-bit Python.)
* **`drain3` moved to an optional `exhaust` extra.** It was a hard dependency
  but is only used by the `tn.mcp.exhaust` template-mining tools, which
  already degrade gracefully when it is absent. `drain3` is sdist-only on
  PyPI (compiled on every install) and hard-pins old `jsonpickle` /
  `cachetools`, which conflicted with common environments. Install the
  exhaust tools with `pip install 'tn-proto[exhaust]'`.
* **`tn.__version__`** is now exposed (sourced from the installed
  distribution metadata).

## [0.6.1] - 2026-06-15 -- First stable on the 0.6 line

Promotes 0.6.0b1 to a stable release. No wire-format or public-API changes
since the beta; the only deltas are CI/test hardening (scoped npm pack
artifact matching, a POSIX state-leak fix in the TS yaml-path portability
test). Published to PyPI (`tn-proto`) and npm (`@cyaxios/tn-proto`) under the
default install tag.
