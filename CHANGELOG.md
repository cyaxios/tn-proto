...
* **TypeScript `tn.seal` / `tn.unseal`.** TypeScript orchestration with
  standard Rust/Wasm JWE and BTN primitives; the application methods retain
  promise-based signatures across Node and browser runtimes. Malformed input
...
* **C# `Tn.SealAsync` / `Tn.UnsealAsync`** via new `tn_runtime_seal` /
  ...
  `TnUnsealException`; `ISealedGroupCipher` lets a host supply an external
  key store or managed second-pass decryptor alongside native JWE support.
...
- **TS BTN rotation now actually works**: pre-0.4.0a3 the TS
  ...
  `BtnPublisher`, swap on disk, bump epoch, attest). JWE rotation archives
  prior reader material, resets to the publisher audience, and requires
  external-reader re-enrollment.
