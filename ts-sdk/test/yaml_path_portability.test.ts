...
test("createFreshCeremony rejects cross-drive yaml paths (Windows guarantee)", () => {
  ...
  try {
    ...
    if (process.platform === "win32") {
      ...
      // On POSIX, "D:\\b\\keys" is not a drive-absolute path; path.relative
      // treats it as a single relative segment, so the cross-drive throw
      // does not fire. The cross-drive rejection is a Windows-specific
      // guarantee. What we CAN assert everywhere: nothing drive-absolute is
      // ever written.
      call();
