...
test("e2e: tn-js init emits a claim URL + flipped .tn/<name>/ layout", gate, async () => {
  ...
  const r = await runCli(["init", "E2eProj"], root, env);
...
test("e2e: tn-js init --no-link mints without a claim URL", gate, async () => {
  const { root, env } = scratch();
  const r = await runCli(["init", "OfflineProj", "--no-link"], root, env);
...
test("e2e: warm-attach attaches a new project with no browser (shared DID)", gate, async () => {
  ...
  const a = await runCli(["init", "ProjA"], root, env);
  ...
  const c = await runCli(["account", "connect", code, "--yaml", yamlA, "--vault", VAULT_URL], root, env);
  ...
  const b = await runCli(["init", "ProjB"], root, env);
