// Smoke test: drive `WasmRuntime` through the `nodeStorageAdapter`
// end-to-end. After the time-source and path-normalization fixes landed,
// emit and read both succeed unconditionally; the test asserts a real
// round-trip.
...
test("WasmRuntime: full init + emit + read round-trip through nodeStorageAdapter", () => {
  ...
  try {
    ...
    // `readWithVerify()`. Asserting the basics here keeps
...
// ---------------------------------------------------------------------------
// Read-variant smokes. Each test sets up its own ceremony so the
...
test("WasmRuntime.readAllRuns: returns an array (surface check)", () => {
...
// ---------------------------------------------------------------------------
// Emit-variant smokes. The severity shorthands (info / warning /
...
test("WasmRuntime: emit verbs (info, warning, log, emitWith) round-trip", () => {
  // Reuses the freshRuntime() helper introduced above -
  // single ceremony, exercise every emit verb against it.
  const rt = freshRuntime();
  ...
  try {
    ...
    // JS surface; deeper signing-flag assertions live in tn-core / PyO3
    // suites. Both must accept
...
// ---------------------------------------------------------------------------
// Admin smokes. Exercises every admin verb on WasmRuntime against a
// fresh ceremony, asserting the underlying keystore_backend now routes
// through the storage adapter (a prerequisite - without it the kit
// + state writes short-circuit straight to std::fs and fail on wasm).
// ---------------------------------------------------------------------------

test("WasmRuntime: admin verbs round-trip", () => {
...
// ---------------------------------------------------------------------------
// Vault link/unlink smokes. These emit admin events; the explicit-path
...
// yaml to route admin events to the main log (same trick the admin-verbs
// test uses) so the emit lands somewhere read() can see.
// ---------------------------------------------------------------------------
...
function freshRuntimeWithAdminInMain(): { rt: WasmRuntime; td: string; yamlPath: string } {
  ...
  const noderuntime = NodeRuntime.init(yamlPath);
  noderuntime.close();
  // Same patch as the admin-verbs test: admin events default to a
...
// ---------------------------------------------------------------------------
// Handler + log-level smokes. Asserts:
...
test("WasmRuntime: init + emit + read + mint, all through memoryStorageAdapter (browser flow)", () => {
  ...
  try {
    ...
    // a None for recipient_identity - pass undefined to mint a kit without
    // recording a recipient identity. Simpler path.
