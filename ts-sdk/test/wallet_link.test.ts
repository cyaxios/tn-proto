...
test("setLinkStateInYaml — mode=linked writes linked_vault + linked_project_id", () => {
  ...
  try {
    ...
    assert.equal(doc.vault.url, "http://vault.example");
    assert.equal(doc.vault.linked_project_id, "proj-xyz");
...
test("setLinkStateInYaml — mode=local clears linked fields", () => {
  ...
  try {
    ...
    assert.equal(doc.vault.enabled, false);
    assert.equal(doc.vault.url, "");
    assert.equal(doc.vault.linked_project_id, "");
    assert.equal(doc.vault.autosync, false);
    assert.equal(doc.vault.sync_interval_seconds, 600);
  } finally {
    rmSync(tmp, { recursive: true, force: true });
  }
...
test("readLinkState — disabled vault block suppresses legacy linked fields", () => {
  const tmp = mkdtempSync(join(tmpdir(), "wallet-disabled-vault-unit-"));
  try {
    const yamlPath = join(tmp, "tn.yaml");
    writeFileSync(
      yamlPath,
      [
        ...
        "  id: local_test03",
        "  mode: local",
        "  linked_vault: http://legacy.example",
        "  linked_project_id: legacy-proj",
        "vault:",
        "  enabled: false",
        "  url: ''",
        "  linked_project_id: ''",
    ...
    assert.equal(state.linkedVault, "");
    assert.equal(state.linkedProjectId, "");
  } finally {
    rmSync(tmp, { recursive: true, force: true });
  }
});
...
test(
  "WalletNamespace.link — creates project + flips yaml to linked",
  { skip: !reachable && "vault not reachable" },
  async () => {
    ...
    try {
      ...
      const result = await WalletNamespace.link(client, yamlPath, {
        projectName: `ts-port-link-${Date.now()}`,
      });
...
test(
  "WalletNamespace.link — second call is idempotent (newlyLinked=false)",
  { skip: !reachable && "vault not reachable" },
  async () => {
    const { tmp, yamlPath, device } = await seedCeremony();
    try {
      const client = await VaultClient.forIdentity(vaultIdentityFromDeviceKey(device), VAULT_URL);
      const first = await WalletNamespace.link(client, yamlPath, {
        projectName: `ts-port-link-idem-${Date.now()}`,
      });
...
test(
  "WalletNamespace.link — re-link to a DIFFERENT vault throws VaultError",
  { skip: !reachable && "vault not reachable" },
  async () => {
    const { tmp, yamlPath, device } = await seedCeremony();
    try {
      const client = await VaultClient.forIdentity(vaultIdentityFromDeviceKey(device), VAULT_URL);
      await WalletNamespace.link(client, yamlPath, {
        projectName: `ts-port-link-diff-${Date.now()}`,
      });
