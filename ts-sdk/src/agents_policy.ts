...
export function parsePolicyText(text: string, path: string): PolicyDocument {
  ...
  const contentHash =
    "sha256:" + createHash("sha256").update(Buffer.from(canonical)).digest("hex");
...
export function policyPathFor(yamlDir: string): string {
  return isAbsolute(POLICY_RELATIVE_PATH)
    ? POLICY_RELATIVE_PATH
    : pathResolve(yamlDir, POLICY_RELATIVE_PATH);
}

/** Load `<yamlDir>/.tn/config/agents.md` if it exists.
 *
 * Returns `null` when the file does not exist (no policy → no splice).
