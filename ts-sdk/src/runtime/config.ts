...
// `me: {did: ...}` to `device: {device_identity: ...}`. Reject the legacy
// block outright so the failure surfaces at init() not at first emit,
// matching python/tn/config.py.
...
// conventions. Only the auto-injected `tn.agents` group is allowed;
