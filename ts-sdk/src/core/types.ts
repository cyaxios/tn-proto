...
export interface RecipientEntry {
  leafIndex: number;
  // Wire/field name matching Python's `tn.admin.recipients()` row contract.
  // This is the recipient's DID (or null if the mint did not name one), but
  // the key is `recipient_identity` to stay byte-for-byte aligned with the
  // Python reference shape — NOT a DID-named field.
  recipient_identity: string | null;
  mintedAt: string | null;
  kitSha256: string | null;
  revoked: boolean;
  // Revocation OR retirement timestamp (Python maps `revoked_at` OR
  // `retired_at` into this single field so retired recipients keep a stamp).
