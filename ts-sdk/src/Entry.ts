...
export class VerifyError extends Error {
  ...
  constructor(sequence: number, event_type: string, failed_checks: string[]) {
    super(
      `entry seq=${sequence} event=${JSON.stringify(event_type)} failed: ` +
        failed_checks.join(", "),
    );
...
/**
 * Shape of the raw `{envelope, plaintext, valid}` triple produced by the
 * reader — accepted by `Entry.fromRaw`. Mirrors `core/types.ReadEntry`,
 * declared inline so this file stays self-contained.
 */
...
function _coerceTimestamp(v: unknown): Date {
...
function _mergeGroupPlaintexts(plaintext: Record<string, unknown>): {
  ...
  for (const gname of Object.keys(plaintext).sort()) {
    ...
    for (const [k, v] of Object.entries(b)) fields[k] = v;
  }
...
function _scanEnvelopeExtras(
  ...
  for (const [k, v] of Object.entries(env)) {
    if (ENVELOPE_BASICS.has(k)) continue;
    if (
      v !== null &&
      typeof v === "object" &&
      !Array.isArray(v) &&
      ...
      const hadKey =
        ...
        (typeof pt !== "object" ||
          pt === null ||
          !((pt as Record<string, unknown>)["$no_read_key"] === true));
      if (!hadKey) hidden.push(k);
      continue;
    }
...
function _formatHHMMSSmmm(ts: Date): string {
...
export class Entry {
   ...
   */
  static fromRaw(raw: RawEntryInput): Entry {
    const env = raw.envelope;
    const plaintext = (raw.plaintext ?? {}) as Record<string, unknown>;
    ...
    const message = _hoistSlot(fields, env, "message", (v) =>
   ...
   * `_valid`) is dropped.
   */
  static fromFlat(d: Record<string, unknown>): Entry {
  ...
  [NODE_INSPECT_CUSTOM](): string {
    ...
    return (
      `Entry(event_type=${JSON.stringify(this.event_type)}, ` +
