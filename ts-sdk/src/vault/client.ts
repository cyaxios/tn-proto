 ...
 *
 * Mirrors `tn.vault_client.VaultClient` from the Python SDK. Covers
 * auth, projects, restore_manifest, the legacy sealed-blob download, and
 * the AWK/BEK whole-body routes.
 */
export class VaultClient {
  ...
  static async forIdentity(
    ...
    // G2: a pre-authenticated session token (arg, or TN_VAULT_SESSION_TOKEN /
    // legacy TN_VAULT_JWT) is used as the bearer directly, skipping the DID
    // challenge/verify dance. This is the headless / non-interactive path.
    const token =
      opts.sessionToken ??
      process.env.TN_VAULT_SESSION_TOKEN ??
      process.env.TN_VAULT_JWT ??
      null;
    const c = new VaultClient({ baseUrl, identity, ...opts, token });
    if (!token && opts.autoAuth !== false) {
  ...
  /**
   * Authenticated GET returning the raw `Response` (auto-reauth on 401).
   * The public counterpart of Python's `client._request("GET", path)`,
   * used e.g. by `tn.auth` for the enrolled check against `/account/me`.
   */
  async get(path: string): Promise<Response> {
    return this._request({ method: "GET", path });
  }
  ...
  // ── AWK/BEK account surface (per-account AWK, per-project BEK) ────
