...
const _FLAT_ENVELOPE_KEYS: readonly string[] = [
  "timestamp",
  "event_type",
  "level",
  "did",
...
const _CRYPTO_ENVELOPE_KEYS: ReadonlySet<string> = new Set([
  "prev_hash",
  "row_hash",
  "signature",
]);

const _RESERVED_ENVELOPE_KEYS: ReadonlySet<string> = new Set([
  ..._FLAT_ENVELOPE_KEYS,
  ..._CRYPTO_ENVELOPE_KEYS,
]);
...
function flattenRawEntry(
  ...
  // 1. Envelope basics.
  for (const k of _FLAT_ENVELOPE_KEYS) {
  ...
  for (const [k, v] of Object.entries(env)) {
    if (_RESERVED_ENVELOPE_KEYS.has(k)) continue;
...
function invalidReasonsFromValid(valid: ReadEntry["valid"]): string[] {
  const out: string[] = [];
 ...
 * concern.
 */
function attachInstructions(flat: SecureEntry, raw: ReadEntry): void {
