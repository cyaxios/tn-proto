...
async function loadBundleIntoMemory(id, label, bundle) {
  ...
  // Index-consistency support: retain the group index master key + ceremony
  // id when the bundle carries them, so verifyBatch can recompute equality
  // tokens from decrypted values. Kit-only bundles simply won't have them
  // (index check then reports "not checked" rather than failing).
  let index_master_key = null;
  for (const [name, valB64] of Object.entries(bundle.files)) {
    if (/(^|\/)index_master\.key$/.test(name)) {
      try { index_master_key = b64ToBytes(valB64); } catch { /* ignore */ }
      break;
    }
  }
  const ceremony_id = bundle.ceremony_id || bundle.ceremony || null;
  unlocked.set(id, { label, kits, index_master_key, ceremony_id });
...
async function popupDecrypt({ ciphertext_b64 }) {
  ...
  const dt = Date.now() - t0;
  console.log(`[ext:popup-decrypt] no kit matched tried=${totalKitsTried} dt=${dt}ms`);
...
// ---------------------------------------------------------------------------
// Envelope verification (integrity + authenticity). Works on any envelope
// found in the page text — decryption is NOT required, so a row you hold no
// kit for can still be proven authentic. Two checks, both via the bundled
// wasm (no JS crypto):
//   * integrity  — recompute row_hash from the wire fields and compare to
//                  the stated row_hash (catches tampering of any field,
//                  ciphertext, or field_hash).
//   * authenticity — verifyDid: the Ed25519 signature over the row_hash was
//                  produced by the publisher's device key.
// Reconstruction of computeRowHash's input from the wire envelope is
// validated byte-for-byte against real SDK output (see the 2026-06-18
// verify_check harness): group blocks -> {ciphertext_b64, field_hashes};
// top-level non-header keys -> public_fields.
// ---------------------------------------------------------------------------
const _RH_HEADER = new Set([
  "device_identity", "did", "timestamp", "event_id", "event_type", "level",
  "sequence", "prev_hash", "row_hash", "signature", "public_fields",
  "group", "ciphertext", "field_hashes", "message",
]);

function _toRowHashInput(env) {
  const groups = {};
  const public_fields = { ...(env.public_fields || {}) };
  for (const [k, v] of Object.entries(env)) {
    if (_RH_HEADER.has(k)) continue;
    if (v && typeof v === "object" && typeof v.ciphertext === "string") {
      groups[k] = { ciphertext_b64: v.ciphertext, field_hashes: v.field_hashes || {} };
    } else {
      public_fields[k] = v; // a top-level cleartext public field
    }
  }
  // Flat single-group shape (top-level ciphertext + group name): reconstruct
  // the one group so synthetic/flat envelopes verify too.
  if (Object.keys(groups).length === 0 && typeof env.ciphertext === "string") {
    groups[env.group || "default"] = {
      ciphertext_b64: env.ciphertext,
      field_hashes: env.field_hashes || {},
    };
  }
  return {
    device_identity: env.device_identity || env.did,
...
function _envelopeCiphertexts(env) {
  const cts = [];
  for (const [k, v] of Object.entries(env)) {
    if (_RH_HEADER.has(k)) continue;
    if (v && typeof v === "object" && typeof v.ciphertext === "string") cts.push(v.ciphertext);
  }
  if (typeof env.ciphertext === "string") cts.push(env.ciphertext);
  return cts;
}

// Sweep every unlocked kit against a group ciphertext. Returns the
// decrypting keystore entry + plaintext bytes, or null. Mirrors the
// popup-decrypt sweep but used here purely to recover the cleartext values
// the equality tokens were derived from.
function _findKitDecrypt(ctBytes, ns) {
  for (const [, ks] of unlocked.entries()) {
    for (const [, kits] of ks.kits.entries()) {
      for (const kit of kits) {
        try { return { ks, plaintextBytes: ns.btnDecrypt(kit, ctBytes) }; }
        catch { /* this kit doesn't open it */ }
      }
    }
  }
  return null;
...
// Index-consistency for one group: recompute each equality token from the
// decrypted value + the group index key and compare to the stated token.
// Rotation epochs aren't carried in the wire row, so we probe a small range:
// the first epoch that reproduces ANY token is taken as the row's epoch
// (every other epoch yields a wholly different key, so a partial match can
// only happen at the right one). all-match -> consistent; partial-match ->
// a lying token; no-match-at-any-epoch -> can't determine (epoch we don't
// hold), reported as null rather than a false alarm.
function _indexCheckGroup(plain, fieldHashes, master, ceremony, groupName, ns) {
  const checkable = Object.keys(fieldHashes || {}).filter((f) => f in plain);
  if (checkable.length === 0) return { index_ok: null, bad: [] };
  for (let ep = 0n; ep <= 4n; ep += 1n) {
    let gk;
    try { gk = ns.deriveGroupIndexKey(master, ceremony, groupName, ep); }
    catch { return { index_ok: null, bad: [] }; }
    let match = 0;
    const bad = [];
    for (const f of checkable) {
      let tok;
      try { tok = ns.indexToken(gk, f, plain[f]); } catch { tok = null; }
      if (tok === fieldHashes[f]) { match += 1; continue; }
      // A whole-number value is ambiguous: JS JSON can't distinguish an int
      // 12 from a float 12.0, and those canonicalize to different tokens. So
      // a mismatch on a whole number isn't necessarily a lie — don't flag it.
      // Strings and fractional numbers have no such ambiguity.
      const v = plain[f];
      const ambiguous = typeof v === "number" && Number.isInteger(v);
      if (!ambiguous) bad.push(f);
    }
    if (match > 0) return { index_ok: bad.length === 0, bad, epoch: Number(ep) };
  }
  return { index_ok: null, bad: [] };
}

async function verifyBatch(envelopes) {
  const t0 = Date.now();
  const ns = await ensureWasm();
  const out = [];
  for (const env of envelopes || []) {
    let hash_ok = false, sig_ok = false, error = null;
    try {
      hash_ok = ns.computeRowHash(_toRowHashInput(env)) === env.row_hash;
    } catch (e) { error = `hash: ${e?.message || e}`; }
    try {
      const sig = ns.signatureFromB64(env.signature);
      sig_ok = ns.verifyDid(env.device_identity || env.did, new TextEncoder().encode(env.row_hash), sig);
    } catch (e) { error = (error ? error + "; " : "") + `sig: ${e?.message || e}`; }

    // Index-consistency: only checkable on group ciphertexts we can decrypt
    // with a kit that also carries an index master key + ceremony id.
    let index_ok = null;
    const index_bad = [];
    try {
      for (const [gname, gval] of Object.entries(env)) {
        if (_RH_HEADER.has(gname)) continue;
        if (!gval || typeof gval !== "object" || typeof gval.ciphertext !== "string") continue;
        let ctBytes;
        try { ctBytes = b64ToBytes(gval.ciphertext); } catch { continue; }
        const dec = _findKitDecrypt(ctBytes, ns);
        if (!dec || !dec.ks.index_master_key || !dec.ks.ceremony_id) continue;
        let plain;
        try { plain = JSON.parse(new TextDecoder().decode(dec.plaintextBytes)); } catch { continue; }
        const r = _indexCheckGroup(plain, gval.field_hashes, dec.ks.index_master_key, dec.ks.ceremony_id, gname, ns);
        if (r.index_ok === true) { if (index_ok === null) index_ok = true; }
        else if (r.index_ok === false) { index_ok = false; index_bad.push(...r.bad); }
      }
    } catch { /* index check is best-effort; never block hash/sig verdict */ }

    out.push({
      hash_ok, sig_ok, index_ok, index_bad_fields: index_bad, error,
      ciphertexts: _envelopeCiphertexts(env),
      publisher_did: env.device_identity || env.did,
      event_type: env.event_type,
      sequence: env.sequence,
    });
  }
  const verified = out.filter((r) => r.hash_ok && r.sig_ok).length;
  const idxChecked = out.filter((r) => r.index_ok !== null).length;
  console.log(`[ext:verify] verifyBatch n=${out.length} verified=${verified} index_checked=${idxChecked} dt=${Date.now() - t0}ms`);
  return { ok: true, results: out };
}
...
// Bounds + shape checks for the externally-supplied pair payload. The
// origin allowlist proves WHO is talking to us; this proves the message
// is WELL-FORMED before any of it reaches storage. A compromised or
// XSS'd vault origin should not be able to write unbounded / arbitrary
// entries into chrome.storage.local.
const MAX_PAIR_KITS = 256;
const PAIR_LABEL_MAX = 256;        // free-text labels (account/project)
const PAIR_KIT_B64_MAX = 64 * 1024; // ~48 KiB of raw bytes per kit/state
const PAIR_ID_RE = /^[A-Za-z0-9._:-]{1,128}$/; // project_id / group

function _isOkLabel(v) {
  return v == null || (typeof v === "string" && v.length <= PAIR_LABEL_MAX);
}

function validatePairKit(k) {
  if (!k || typeof k !== "object") return "kit not an object";
  if (typeof k.project_id !== "string" || !PAIR_ID_RE.test(k.project_id)) return "bad project_id";
  if (typeof k.group !== "string" || !PAIR_ID_RE.test(k.group)) return "bad group";
  const kitB64 = k.kit_bytes_b64 || k.kit_b64;
  if (typeof kitB64 !== "string" || kitB64.length === 0 || kitB64.length > PAIR_KIT_B64_MAX) return "bad kit bytes";
  const stateB64 = k.state_bytes_b64 || k.state_b64;
  if (stateB64 != null && (typeof stateB64 !== "string" || stateB64.length > PAIR_KIT_B64_MAX)) return "bad state bytes";
  if (!_isOkLabel(k.project_label)) return "bad project_label";
  return null;
}

function validatePairMessage(msg) {
  if (!Array.isArray(msg.kits) || msg.kits.length === 0) return "kits must be a non-empty array";
  if (msg.kits.length > MAX_PAIR_KITS) return `too many kits (max ${MAX_PAIR_KITS})`;
  if (!_isOkLabel(msg.account_label)) return "bad account_label";
  for (const k of msg.kits) {
    const why = validatePairKit(k);
    if (why) return why;
  }
  return null;
}
...
chrome.runtime.onMessageExternal.addListener((msg, sender, sendResponse) => {
  (async () => {
    try {
      ...
      const invalid = validatePairMessage(msg);
      if (invalid) {
        console.error(`[ext:vault-pair] rejecting malformed payload: ${invalid}`);
        return sendResponse({ ok: false, reason: `invalid pair payload: ${invalid}` });
      }
      const kits = msg.kits;
  ...
  (async () => {
    try {
      switch (msg.type) {
        ...
        case "verifyBatch":         return sendResponse(await verifyBatch(msg.envelopes));
