...
// Where to open the pair flow: the production vault (vault.tn-proto.org is
// the deployed Worker that serves /extension-pair). This origin is also the
// sole entry in externally_connectable + EXT_PAIR_ALLOWED_ORIGINS, so it is
// the only origin allowed to post kits back to the extension.
...
async function setupExtensionUnlock(kind) {
  ...
  try {
    if (kind === "prf") {
      ...
      const salt = rand(16);
      // OWASP 2023 floor for PBKDF2-HMAC-SHA256. Unlock re-derives with
      // the iteration count persisted in the verifier meta, so raising
      // this only affects newly-created extension passphrases.
...
// activeTab model: the content script is NOT auto-injected on every page.
// It is injected on demand when the user clicks "Decrypt this page" (or
// right after unlocking), which is a user gesture that grants activeTab
// host access to the current tab only. Re-injection is idempotent — the
// `window.__tn_decrypt_injected` guard in content.js makes the second
// inject a no-op — so we always follow up with a "rescan" message to kick
// an already-present instance.
async function decryptActivePage() {
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (!tab?.id) return;
  try {
    await chrome.scripting.executeScript({ target: { tabId: tab.id }, files: ["content.js"] });
  } catch (e) {
    // Restricted surfaces (chrome://, the Web Store, view-source, PDF
    // viewer, other extensions' pages) can't be injected. Surface a hint
    // rather than failing silently.
    console.error(`[ext] cannot run on this page: ${e?.message || e}`);
    const banner = $("banner");
    if (banner) { banner.className = "banner warn"; banner.textContent = "Can't decrypt this page (restricted by the browser)."; }
    return;
  }
  chrome.tabs.sendMessage(tab.id, { type: "rescan" }).catch(() => {});
}

$("btn-rescan").addEventListener("click", decryptActivePage);
...
// ---------------------------------------------------------------------------
// Collapsible keystores section. Minimized by default; the open/closed
// choice is persisted in chrome.storage.local so it survives popup re-opens.
// renderKeystores() only replaces #keystores innerHTML, never its class, so
// the collapse state is stable across refreshes.
// ---------------------------------------------------------------------------
const KS_STATE_KEY = "keystores_expanded";

function setKeystoresCollapsed(collapsed) {
  $("keystores").classList.toggle("collapsed", collapsed);
  const chev = $("ks-chevron");
  if (chev) chev.textContent = collapsed ? "▸" : "▾"; // ▸ / ▾
}

async function applyKeystoresCollapseState() {
  const got = await chrome.storage.local.get(KS_STATE_KEY);
  // Default to collapsed when there is no saved preference.
  setKeystoresCollapsed(got[KS_STATE_KEY] !== true);
}

$("keystores-header").addEventListener("click", async () => {
  const wasCollapsed = $("keystores").classList.contains("collapsed");
  setKeystoresCollapsed(!wasCollapsed);
  // wasCollapsed === true means we just expanded it -> persist expanded.
  await chrome.storage.local.set({ [KS_STATE_KEY]: wasCollapsed });
});

// ---------------------------------------------------------------------------
// Log out — sign out of the vault by dropping the vault-paired kits (they can
// be re-pulled anytime via "Sign in with vault"); disk-imported kits are kept.
// Then lock everything. NOTE: "log out" semantics are a guess — confirm intent.
// ---------------------------------------------------------------------------
$("btn-logout").addEventListener("click", async () => {
  const s = lastStatus || (await chrome.runtime.sendMessage({ type: "status" }));
  const paired = (s.keystores || []).filter((k) => (k.source || "vault") === "vault-paired");
  const msg = paired.length
    ? `Sign out of the vault? This removes ${paired.length} vault-paired kit${paired.length === 1 ? "" : "s"} (re-pair anytime). Kits imported from disk are kept.`
    : "Lock all keystores?";
  if (!confirm(msg)) return;
  for (const k of paired) {
    await chrome.runtime.sendMessage({ type: "removeKeystore", id: k.id });
  }
  await chrome.runtime.sendMessage({ type: "lockAll" });
  await refresh();
});
...
// raw JSON blob.
// Kept in lockstep with content.js::_isTnEnvelope — accept either
// `device_identity` or `did`, and `sequence` as number OR string. A
// narrower check here used to make the same envelope render as a TN
// entry in the page but as raw JSON in the popup.
...
refresh().then(applyKeystoresCollapseState);
