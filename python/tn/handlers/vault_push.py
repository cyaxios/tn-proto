...
The push side subscribes to admin events. On write (or on a schedule),
runs ``tn.export(out_path, kind="admin_log_snapshot")`` and POSTs the
resulting ``.tnpkg`` to the vault inbox addressed by the writer's DID.

Two operating modes, distinguished by ``sync_state.account_bound``:
...
* **INIT-UPLOAD mode** (``sync_state.account_bound is False``): the
  ...
  BEK travels in the URL fragment to the browser claim page; the
  vault stores ciphertext only. Persisted to sync_state so a
...
The bulk of the push work is also exposed as a module-level pure
function ``push_snapshot(cfg, client, *, scope="admin",
...
def push_snapshot(
    ...
    *,
    ...
    # Admin snapshots stage at <yaml_dir>/.tn/<stem>/admin/outbox/ (the
    # unified per-stem layout). The legacy path was
    # <yaml_dir>/.tn/admin/outbox/; see legacy_admin_outbox_dir for the
    # read-side fallback used by earlier-staged-but-not-yet-POSTed
    # snapshots.
...
# ---------------------------------------------------------------------------
# Init-upload mode: the unauthenticated /pending-claims path taken when
# the ceremony has no vault account bound yet.
# ---------------------------------------------------------------------------
...
def _emit_claim_url_admin_event(
    ...
    *,
    ...
    per-stem admin outbox (``.tn/<stem>/admin/outbox/``).

    Invariant C17 in ``live_consistency_check.py`` checks the
    ``admin/outbox/`` directory specifically. We write a minimal
    envelope JSON file there — the same shape an admin event would have
    on the main log — so an auditor inspecting the outbox sees the
    issuance trail. Best-effort: a write failure is logged and swallowed.
    """
    from ..conventions import admin_outbox_dir
...
def init_upload(
    ...
    *,
    ...
    # POST to /api/v1/pending-claims (no auth — this is the init-upload path).
    ...
    # Build the claim URL. Fragment carries the BEK; the server never
    # sees this value. ``k=`` prefix matches the browser claim page's
...
class VaultPushHandler(TNHandler):
    ...
    def __init__(
        ...
        *,
        ...
        # Init-upload serialization. Two concurrent
    ...
    def emit(self, envelope: dict[str, Any], raw_line: bytes) -> None:
        if self._trigger == "on_emit":
            try:
                ...
                # A failed push must not take down the caller. The next
                # write / scheduled tick retries.
    ...
    def _push_snapshot(self) -> bool:
        ...
        The entry point switches between init-upload mode (no JWT yet —
        POST to /pending-claims unauth) and steady-state mode (POST to
        /inbox/{did}/snapshots/...) based on
        ``sync_state.account_bound``. The dispatch happens here so
        ...
        persisted to ``<yaml_dir>/.tn/sync/state.json`` so
        ...
        # Mode dispatch. account_bound flips True after the
    ...
    def _init_upload_tick(self, cfg: Any) -> bool:
        ...
        When the vault is unreachable, HTTP errors are soft-warned and
        swallowed; the next handler tick retries.

        Concurrency contract: the entire init pass — mint of
        ...
        # by design) but we keep the same client factory so tests
        ...
        with self._init_lock:
            ...
            try:
                ...
                except Exception as e:  # noqa: BLE001 — soft warning; retried next tick
                    _log.warning(
...
def _default_client_factory(endpoint: str, identity: Any) -> Any:
    ...
    The handler runs in two modes. The init-upload mode hits
...
class _SnapshotPostingClient:
    ...
    ``post_pending_claim`` is the init-upload UNAUTHENTICATED POST.
    """
    ...
    def post_pending_claim(
        ...
        """Unauthenticated POST to /api/v1/pending-claims.

        Returns the JSON body ``{"vault_id": ..., "expires_at": ...}``.
        Does not send any Authorization header — the endpoint is
        explicitly unauthenticated.

        Sends ``X-Publisher-Did`` so the vault can later send a
        ``contact_update`` tnpkg back to this package's inbox at bind
        time. The header is optional for backward compatibility — the
        endpoint just stores ``None`` and skips the send if it's missing.
