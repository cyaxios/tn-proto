...
The pull side runs on a schedule, GETs new ``.tnpkg`` files from the
vault inbox addressed to this DID, and calls ``tn.absorb()`` for each.
Idempotent (absorb dedupes by ``row_hash`` so re-fetching a snapshot is
a no-op).

One-shot entry point
--------------------

The bulk of the pull work is also exposed as a module-level
...
def pull_inbox(
    ...
    *,
    ...
    for item in items:
        path = item.get("path")
        # Advance the cursor by the server-supplied since_marker when
        # present (opaque, order-preserving). Fall back to received_at
        # for backward compat with vault implementations that don't
        # supply since_marker yet.
...
class VaultPullHandler(TNHandler):
    ...
    def _load_cursor(self, cfg: Any) -> dict[str, Any]:
        # Prefer the unified sync state file.
