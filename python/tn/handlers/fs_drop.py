...
Useful for offline or air-gap scenarios. The handler subscribes to
admin events. On each write (after filtering) it composes
``tn.export(out_path, kind="admin_log_snapshot")`` and the resulting
``.tnpkg`` is dropped into a local outbox directory. A peer's
``FsScanHandler`` (or any other process) picks the file up and
absorbs it.
