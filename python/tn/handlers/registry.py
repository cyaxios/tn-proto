...
``vault.sync``                             tn-proto.org cloud vault
``vault.push``                             admin snapshot push
``vault.pull``                             admin snapshot pull
``fs.drop``                                local outbox drop
``fs.scan``                                local inbox scan
``stdout``                                 write JSON envelope lines to stdout
=========================================  =======

Filter specs are parsed by :func:`tn.filters._compile_filter` which
handles both the classic field/op dict shape and the shorthand
