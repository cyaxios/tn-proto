...
Status: this is the handler SCAFFOLD. The following pieces are
wired:
...
The following pieces are intentionally stubbed and ship later:

* Real WebSocket client (open, reconnect-with-backoff, send).
* Frame encryption with project BEK from keystore. For now the handler
  uses a test-stub BEK source so it can be exercised end-to-end before
  the real keystore integration and manifest schema changes land.
...
Config shape (``tn.yaml``)::
    ...
    handlers:
      - kind: tn.firehose
        ...
        key_id: fhk_<base32>             # optional while the endpoint is a test stub
        filter:
...
def _stub_bek(project_id: str, key_id: str | None) -> bytes:
    ...
    **This is not secure.** Two devices with the same project_id and
    ...
    The real implementation replaces this function with
    ``keystore.lookup_bek(key_id)`` returning bytes provisioned via the
    mint flow.
    """
...
# follow a "fhk_<base32>" shape, but the scaffold accepts anything for
# test-stub purposes.
...
class TnFirehoseHandler(AsyncHandler):
    ...
    key_id
        ...
        frames with. For now this is a test-stub identifier; the real
        path wires BEK lookup from the local keystore.
    filter_spec
    ...
    def __init__(
        ...
        *,
        ...
        # Test-stub BEK. The real path swaps for keystore lookup by key_id.
    ...
    def _ws_url(self) -> str:
        ...
        ``wss://vault.tn-proto.org/firehose/00000000-...``. For now the
        path matches what the prototype Worker exposes; production will
        route via the vault, which proxies upstream to the Worker.
        """
    ...
    def _ensure_connected(self) -> Any:
        ...
        with self._ws_lock:
            ...
            ws = websocket.create_connection(
                ...
                # the scaffold connects unauthenticated against the
