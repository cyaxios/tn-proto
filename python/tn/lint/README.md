...
The fixtures under [tests/fixtures/](tests/fixtures/) cover one violation per rule plus a clean baseline. Their `tn.yaml` extends the included [test policy](tests/fixtures/packs/test-policy.yaml).
...
| `rules.py` | `Rule` protocol and `R1`-`R3` implementations. |
