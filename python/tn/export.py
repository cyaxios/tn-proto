...
def _pack_stream_yamls(cfg: LoadedConfig) -> dict[str, bytes]:
    ...
    Layouts, newest first:
      * Project-root overlays: ``<project>/streams/<name>.yaml``
      * Older ceremony layout: sibling ``<name>/tn.yaml`` dirs under the
        project root (``<root>/default/tn.yaml`` -> root is parent.parent)
    ...
    if streams_dir.is_dir():
        for stream_yaml in sorted(streams_dir.glob("*.yaml")):
            if stream_yaml.stem == DEFAULT_CEREMONY_NAME:
                continue
            out[f"body/streams/{stream_yaml.stem}/tn.yaml"] = stream_yaml.read_bytes()
...
def _encrypt_body_in_place(
    ...
    Uses a combined-blob layout (one ``body/encrypted.bin``) rather than
    per-file encryption: it is simpler and matches the "vault sees one
    opaque blob" spirit.
...
def decrypt_body_blob(blob: bytes, key: bytes) -> dict[str, bytes]:
    ...
    Note:
        ...
        binary frame for ciphertexts produced before the STORED-zip
        format - kept until the next state wipe.
    ...
    # LEGACY-COMPAT — drop after next state wipe.
    # Custom binary frame produced by an earlier _encrypt_body_in_place
