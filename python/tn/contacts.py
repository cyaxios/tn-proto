"""Contact-book reducer for ``contact_update`` tnpkgs.

See spec §4.10 / §4.6. The publisher's ``tn sync`` pulls
...
``tn.conventions._stem_dir``). The schema is a simple flat form::
...
account). Match → replace in place. No match → append.
...
# null; we still demand the keys so downstream code can rely on key
# existence without `.get(...)` everywhere.
...
def _apply_contact_update(yaml_path: Path, body: dict[str, Any]) -> dict[str, Any]:
    ...
    Idempotency rule: match on ``(account_id, package_did)``; replace in
    place if matched, else append. Returns the new doc as written.
    """
...
# ---------------------------------------------------------------------------
# Bulk address resolver (federation work)
# ---------------------------------------------------------------------------
#
