...
def _project_seed_vault_yaml_patch(
    ...
    changed = False
    for field in ("url", "linked_project_id"):
        if not _nonempty_str(patched_vault.get(field)):
            incoming_value = _nonempty_str(incoming_vault.get(field))
            if incoming_value:
                patched_vault[field] = incoming_value
                changed = True
    if changed:
