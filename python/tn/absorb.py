...
def _maybe_unseal_recipient_wrap(
    ...
    * ``state.body_encryption.recipient_wraps`` — plural, array.
      The federation/multi-recipient form: the producer emits one entry
      per recipient key. The consumer walks the array and uses the entry
      whose ``recipient_did`` matches this device.
...
def _absorb_contact_update(
    ...
    Idempotency is keyed on ``(account_id, package_did)``. Errors
