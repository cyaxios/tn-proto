...
# Package version, sourced from the installed distribution metadata so it
# stays in lockstep with pyproject's [project].version without a second
# place to bump. Falls back to "0+unknown" for an editable/source checkout
# that was never `pip install`-ed (no dist metadata on disk).
try:
    from importlib.metadata import PackageNotFoundError as _PkgNotFound
    from importlib.metadata import version as _pkg_version

    try:
        __version__ = _pkg_version("tn-proto")
    except _PkgNotFound:
...
def _init_impl(
    ...
    *,
    ...
    # tn.read() can default-filter to "this run only".
...
# --------------------------------------------------------------------------
# Multi-ceremony module verbs - see tn._multi.
...
__all__ = [  # noqa: RUF022 — intentional category grouping (see inline comments)
    ...
    "agents",
    # auth namespace (account / session / device enrollment — tn.auth.*)
    "auth",
    "AuthError",
    "AuthState",
    "Verdict",
