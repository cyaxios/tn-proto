...
DEFAULT_PUBLIC_FIELDS = [
    ...
    "reason",
    "unlinked_at",
    # Agents-policy + tampered-row admin events. These fields must stay
    # public on the envelope so the policy published / tampered-row events
    # are auditor-replayable without any reader kit.
    "policy_uri",
    "content_hash",
    "event_types_covered",
    "policy_text",
...
class LoadedConfig:
    ...
    def protocol_events_location(self) -> str:
        ...
        The field was originally named ``protocol_events_location`` and
...
def create_fresh(
    ...
    *,
    ...
    # Carries the six policy fields the markdown loader splices into emit
    # when ``.tn/config/agents.md``
    ...
    doc = {
        ...
        "ceremony": {
            ...
            # fold them back into the main log (the original behavior).
...
def _validate_load_doc_structure(yaml_path: Path, doc: Any) -> None:
    ...
    # protocol-level conventions. The only allowed name in the reserved
    # namespace is the auto-injected ``tn.agents`` group.
