...
Implements the CLI side of the loopback dance: the browser does the
WebAuthn-PRF unwrap; we receive the raw BEK over loopback, fetch the
encrypted blob from the vault, AES-GCM decrypt, and write the resulting
bytes to the user's chosen output directory.

Restore is account-bound (not package-bound) and rides the same
account/project key hierarchy as the rest of the vault: a per-account AWK
wraps a per-project BEK. Sync is handler-driven.
"""
...
def _decrypt_blob_with_bek(
    ...
    * **nonce-prefixed** (legacy/simple): ``blob`` is
    ...
    if nonce_b64:
        ...
        # Frame-shaped writers (ts-sdk wallet_sync.ts pushCeremonyBody and
        # its live tests) store the WHOLE nonce||ct frame in ciphertext_b64
        # and duplicate its first 12 bytes as nonce_b64 because the vault's
        # envelope schema requires the field. A genuine browser-minted
        # separate-nonce row never prefixes the nonce onto the ciphertext,
        # so nonce == blob[:12] discriminates the frame shape
        # deterministically (12-byte collision odds are negligible).
        if len(blob) >= 12 + 16 and blob[:12] == nonce:
...
def _try_unpack_export_frame(plaintext: bytes) -> dict[str, bytes] | None:
    ...
    The current plaintext is a STORED zip; we try that first by sniffing
    the ``PK\\x03\\x04`` magic. If the magic doesn't match we fall back to
    an older custom binary frame, kept here as legacy compatibility for
    in-flight bodies (drop after next state wipe):
