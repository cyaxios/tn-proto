...
**DEPRECATED, pending review.** The per-file sealing model implemented
here is superseded by the whole-body encryption model in the new vault
flow (a per-account AWK wrapping a per-project BEK). New code MUST NOT
call this module's ``seal()`` / ``_unseal()``. Existing callers and
tests stay until the legacy ``PUT /api/v1/projects/{id}/files/{name}``
route is removed in a future cycle. The TS / Rust ports of this module
are explicitly NOT being built; that workstream is dropped.
