...
def test_env_format_is_paste_able_kv_lines(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("TN_VAULT_URL", "https://vault.example")
    monkeypatch.setenv("TN_VAULT_SESSION_TOKEN", "live-secret-value")
    ...
    assert "TN_VAULT_SESSION_TOKEN=live-secret-value" in text
...
def test_json_format_redacts_secrets(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("TN_VAULT_SESSION_TOKEN", "shouldnotappear")
    text = _render_json(
        _ENV_SCHEMA, dict(__import__("os").environ), {}, redact_secrets=True
    )
    ...
    parsed = json.loads(text)
    by_name = {row["name"]: row for row in parsed["entries"]}
    token_row = by_name["TN_VAULT_SESSION_TOKEN"]
    assert token_row["secret"] is True
    assert token_row["source"] == "env"
    assert token_row["value"] == _redact("shouldnotappear")
