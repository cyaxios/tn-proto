...
Mirrors test_jwe_roundtrip.py for the pipeline half. The coverage half pins
the contract-D4 design rule: the HIBE-wrapped CEK lives INSIDE the group's
``ciphertext`` blob, so `row_hash` covers it with zero hash-function change —
stripping the group, swapping the blob for one sealed under a different
authority, or flipping any byte must each break verification, and the group
dict must carry no sibling keys.
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="hiberoundtrip_") as td:
        ws = Path(td)
        ...
        print("pipeline: 3/3 entries signed, hashed, chained, decrypted under hibe")
        ...
        tn.flush_and_close()
        print("coverage: strip/swap/flip each break row_hash; no sibling keys")
