"""Tests for the ``contact_update`` tnpkg + vault as publisher.
...
def test_signed_contact_update_manifest_round_trips(tmp_path: Path):
    ...
    pkg_bytes = _signed_contact_update_zip(
        signer=signer,
        recipient_identity="did:key:z6MkRecipient",
        body=_valid_body(),
    )
...
def test_absorb_rejects_signature_tampering(tmp_path: Path):
    ...
    # Manifest is unchanged so it still verifies — the wire format does
    # not build Merkle commitments over body bytes, so tampering the body
    # without re-signing is allowed. We verify here
