...
Regression for a previously-broken end-to-end flow: a publisher
...
``C:\\codex\\tnstage\\cash_register\\``, with two fixes applied: the
admin reducer must read ``all_runs=True``, and ``read_as_recipient``
must be btn-aware.
...
def test_stage6_cross_publisher_btn(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # makes export silently bundle the wrong file.
    ...
    # this process (admin reducer must read all_runs).
...
def test_stage6_recipients_persists_across_processes(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Regression: a recipient added in one process must still
