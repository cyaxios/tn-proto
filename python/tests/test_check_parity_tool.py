...
def test_genuine_gap_entries_have_specific_nongeneric_reasons():
    # Finding 4: the real Python-only TS gaps must NOT be silenced by the
    ...
    # NOTE: `wallet` left this set when the TS SDK shipped the wallet
    # namespace (walletNamespace / walletStatus / walletSyncCmd); it is now
    # a namespace asymmetry, not a gap.
    allowlist = cp.build_allowlist()
    for verb in ("vault_client", "classifier", "is_keystore_diverged"):
    ...
    # wallet stays allowlisted (module re-export) but is NOT a gap any more.
    assert "wallet" in allowlist
    assert allowlist["wallet"].gap is False
    assert allowlist["wallet"].reason not in (cp._LEGACY_REASON, cp._BROWSER_ONLY_REASON)
...
def test_cli_default_summary_surfaces_browser_stub_and_ts_gaps():
    ...
    # (`wallet` left the gap set when the TS wallet namespace shipped.)
