...
Acceptance bar: the JS dashboard's snapshot_builder must produce bytes
that the Python SDK reads/verifies, AND wasm-side encryption must yield
ciphertext that `tn_btn.decrypt` reads.
...
def _ensure_wasm_artifacts() -> Path:
    ...
    if not WASM_TEST_PATH.exists():
        pytest.skip(
            "browser wasm e2e runner not present (cross-repo dependency, "
            f"absent on CI): {WASM_TEST_PATH}"
        )
    proc = subprocess.run(
...
def test_python_reads_browser_built_tnpkg_and_verifies_signature():
    ...
    confirms against publisher_identity.
    """
    interop = _ensure_wasm_artifacts()
...
def test_browser_extracts_package_key_from_python_built_full_keystore_body(tmp_path):
    ...
    This is the headline assertion that the sessionStorage package-key
    stopgap is replaced with real body parsing.
    ...
    if not runner_path.exists():
        pytest.skip(
            f"browser runner not present (cross-repo dependency, absent on CI): {runner_path}"
        )
