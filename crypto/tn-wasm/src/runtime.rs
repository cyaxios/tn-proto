...
//!
//! Phase B1 (MVP) surface per `plans/2026-05-13-wasm-widen-and-fallback-deprecate.md`:
...
//!
//! Phase 2 (read variants) additions:
...
//!   - emit variants (emitWith / overrideSign / log/debug/info/...) — Phase 3
//!   - admin verbs — Phase 4
//!   - handlers + log-level statics — Phase 5
//!   - vault + readFrom — Phase 6
//!   - browser Storage adapter — Phase 7
//!
...
impl WasmRuntime {
    ...
    // sidecar) is routed through the runtime's `Storage` handle as of
    // the Phase 4 keystore refactor, so admin verbs work against the
    // same `JsStorageAdapter` the rest of `WasmRuntime` uses. Without
    // that refactor `admin_add_recipient` would short-circuit the
    // storage abstraction and call stubbed `std::fs::*` on wasm.
    //
    ...
    // -----------------------------------------------------------------
    // Phase 6: vault verbs + read-from-path variants
    //
    ...
    // avoid stomping on the Phase 5 handler / log-level methods that
    // land in parallel.
    // -----------------------------------------------------------------
...
// -----------------------------------------------------------------
// Phase 5: log-level control (static methods)
//
