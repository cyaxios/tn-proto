//! Folding admin envelopes into the materialized [`AdminStateCache`] state.
//!
//! Holds the dispatch orchestrator (`apply_envelope`), the cross-cutting
//! prelude checks (coordinate-fork detection, vector clock), and the
//! per-event-type handlers that each own the projection of one `event_type`
//! onto `self.state`.
