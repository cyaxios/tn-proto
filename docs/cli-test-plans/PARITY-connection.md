...
| 7 | request headers: **`User-Agent: tn-proto/<ver>` only** `vault_client.py:65,136` | `User-Agent: tn-proto-ts/<ver>` + `Accept` + `Content-Type` `account/index.ts:22-26,160` | MATCH | ~~P3~~ RESOLVED | Both sides now send a dynamic versioned UA: TS sends `tn-proto-ts/<version>` from the shared `src/version.ts` helper (landed this release), mirroring Python's `tn-proto/<version>`. TS still adds explicit `Accept`/`Content-Type`; both bodies are JSON. |
