...
| 23 | wrapped-key fetch: `_fetch_wrapped_key` raises RestoreError on ANY non-200 wallet_restore_passphrase.py:152-157; caught → `wrapped=None` → MINT wallet_push.py:359-362 | `getWrappedKey` throws; **only `status===404` → mint, any other status rethrows** wallet_sync.ts:421-424 | DIVERGE | S1 | Python treats EVERY non-200 wrapped-key response (500, 403, 502…) as "no key → MINT a fresh BEK". TS mints only on 404 and propagates other errors. A transient 5xx makes Python mint a NEW BEK and overwrite the wrapped-key, **orphaning the existing body blob** (decryptable only under the old BEK); TS aborts safely. This is the most dangerous divergence. |
