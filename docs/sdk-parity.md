...
| Python                      | TS                            | Status | Notes |
|-----------------------------|-------------------------------|--------|-------|
| `tn init <project>`         | `tn-js init`                  | ✓      | Scaffold-from-scratch flow on both CLIs; non-TTY safe (mnemonic to identity.json). |
| `tn add_recipient <g> <did>`| `tn-js admin add-recipient`   | ✓      | Same shape, different verb naming convention per language. |
| `tn bundle <did> <out>`     | `tn-js bundle <did> <out>`    | ✓      | Both accept `--seal-for-recipient`. |
...
| `tn absorb <pkg>`           | `tn-js absorb <pkg>`          | ✓      | Library API + `Tn.absorb` factory also cover it. |
...
| `tn wallet ...`             | `tn-js wallet ...`            | ✓      | sync / status / pull-prefs / export-mnemonic on both sides. |
